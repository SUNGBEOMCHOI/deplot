...
class LayoutModel:
    def __init__(self, save_json_root_path=None, save_image_root_path=None):
        ...
        self.model = lp.Detectron2LayoutModel(
            ...
            model_path=model_path,    
            #label_map   ={0: "topic", 1: "title", 2: "contents", 3:"figure", 4:"graph",5:"table",6:"caption",7:"comments",8:"images"}, # In model`label_map`
            extra_config=["MODEL.ROI_HEADS.SCORE_THRESH_TEST", 0.8] # Optional
        )
        self.domain = None
        self.save_json_root_path = save_json_root_path
        self.save_image_root_path = save_image_root_path
    ...
    def set_domain(self, domain):
        self.domain = domain
        if self.save_json_root_path is None:
    ...
    def __call__(self, data_path:str):
        ...
        elif data_path.endswith('jpg') or data_path.endswith('png'):
            image = Image.open(data_path)
            image_list = [(data_path, image)]
        else:
...
if __name__ == "__main__":
    save_json_root_path = './'
    save_image_root_path = './'
    layout_model = LayoutModel(save_json_root_path, save_image_root_path)
    layout_model.set_domain("kostat")
    
    image_path = "/inference/test_data/(별첨)2023년도 에너지총조사 조사실시안내 인포그래픽.pdf_1.jpg"
