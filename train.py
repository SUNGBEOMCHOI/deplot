...
def train(model,
        epochs=50,
        image_dir="/root/ChartQA/ChartQA Dataset/test/png", 
        csv_dir="/root/ChartQA/ChartQA Dataset/test/tables"):
    train_dataset = ImageCaptioningDataset(image_dir, csv_dir, processor)
        ...
        for idx, batch in enumerate(tqdm.tqdm(train_dataloader)):
            with accelerator.accumulate(model):
                ...
                if (epoch + 1) % 20 == 0:
                    ...
                    model.train()
        if (epoch + 1) % 50 == 0:
            model.save_pretrained(f'{epoch+1}_model.pt')
...
def test(model,
        image_dir="/root/ChartQA/ChartQA Dataset/translate_test/png",
        csv_dir="/root/ChartQA/ChartQA Dataset/translate_test/tables"):
    train_dataset = ImageCaptioningDataset(image_dir, csv_dir, processor)
    train_dataloader = DataLoader(train_dataset, shuffle=False, batch_size=1, collate_fn=collator, num_workers=4)
    model.eval()
    total_loss = 0.0
    all_predictions = []
    all_labels = []
...
if __name__ == "__main__":
    ...
    # processor.save_pretrained(f'final_model')
    ...
    epochs = 1000
    train(model, epochs, image_dir=train_image_dir, csv_dir=train_csv_dir)
    test(model, image_dir=test_image_dir, csv_dir=test_csv_dir)